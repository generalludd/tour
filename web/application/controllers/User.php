...
class user extends MY_Controller {
	...
	function edit_password($id): void {
		$user_id = $this->session->userdata("user_id");
		if ($id == $user_id || $user_id == 1) {
			$data["id"] = $id;
			$data["target"] = "auth/changepass";
	...
	function create() {
		if ($this->session->userdata("role") == "admin") {
			...
			$data["roles"] = get_keyed_pairs($roles, ["value", "name"]);
			$user_status = $this->variable->get_pairs("user_status");
			$data["user_status"] = get_keyed_pairs($user_status, ["value", "name"]);
			$data["user"] = NULL;
	...
	function update_value() {
		$id = $this->input->post("id");
		$values = [$this->input->post("field") => $value = $this->input->post("value")];
